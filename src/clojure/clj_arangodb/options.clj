(ns clj-arangodb.options
  (:require [clojure.string :as str]))

(defn- class->symbol [^Class c]
  (symbol (.substring (.toString c) 6)))

(def ^:private buildable? (some-fn map? sequential?))
...
(defn build
  "
  Takes a `class` and `map` or `sequence` of pairs of options.
  Creates and calls a sequence of functions a new `object` of class `class`.
  if `options` is not a map or sequence then the value is returned `as is`.
  "
  [class options]
  (if (buildable? options)
    (reduce (fn [acc f]
